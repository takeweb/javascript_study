...
const centerCircle = (context) =>{
  ...
  return () =>{
    ...
    ctx.arc(0, 0, centerCircleData.hankei - centerCircleData.width,
            0, Math.PI * 2);
    ctx.fill();
  };
};
...
const handObj = function(handData, context, hankei) {
  ...
  [pathCtx.lineWidth, pathCtx.strokeStyle] =
    [handData.width, handData.color];
...
handObj.prototype = {
  rewrite : function(val) {
    ...
    [ ctx.lineWidth , ctx.strokeStyle ] =
      [ this.handData.width , this.handData.color ];
    if ( val !== 0 ) {
